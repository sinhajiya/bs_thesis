...
def evaluate_eer_utterance_old(loader, model, device):
# save logits per segment and then avg it to find the score and then find eer
    ...
    print("score min:", final_scores.min())
    print("score max:", final_scores.max())
    print("sample scores:", final_scores[:20])
...
def evaluate_eer_utterance(loader, model, device):
    ...
    print("I evaluate EER per utterance, for each segment in the utterance, i have the logits, i stack them and take the avg lopgit and use the logit for predicting the label")
    model.eval()
    ...
    print("score max:", final_scores.max())
    print("sample scores:", final_scores[:20])
    real_scores = final_scores[final_labels == 0]
    fake_scores = final_scores[final_labels == 1]

    eer, threshold = compute_eer(-real_scores, -fake_scores)

    eer = eer * 100
    preds = (final_scores >= threshold).astype(int)
    cm = confusion_matrix(final_labels, preds)
